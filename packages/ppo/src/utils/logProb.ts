...
export function computeLogProb(actions: tf.Tensor, mean: tf.Tensor, std: tf.Tensor): tf.Tensor {
  return tf.tidy(() => {
    const logUnnormalized = tf.mul(
      -0.5,
      tf.square(tf.sub(tf.div(actions, std), tf.div(mean, std))),
    );
    const logNormalization = tf.add(tf.scalar(0.5 * Math.log(2.0 * Math.PI)), tf.log(std));
...
export function computeLogProbTanh(
  ...
  return tf.tidy(() => {
    ...
    const logGaussian = tf.scalar(-0.5).mul(zSquared.add(logTwoPi).add(logVar)); // shape: [batch, actDim]

    // Jacobian correction: -log(1 - a²)
    const logJacobian = tf.scalar(1).sub(clipped.square()).add(eps).log(); // log(1 - a²)
