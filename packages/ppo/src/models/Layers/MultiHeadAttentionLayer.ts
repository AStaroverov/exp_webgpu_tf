...
export class MultiHeadAttentionLayer extends tf.layers.Layer {
  ...
  call(inputs: tf.Tensor | tf.Tensor[]) {
    return tf.tidy(() => {
      ...
      if (!validLength) {
        throw new Error(
          `MultiHeadAttentionLayer expects ${this.selfAttn ? "2-3 inputs: [qTok, qMask, kvMask?]" : "4 inputs: [qTok, qMask, kvTok, kvMask]"}, got ${inputArray.length}`,
        );
      }
      ...
      const split = (t: tf.Tensor) =>
        t.reshape([B, t.shape[1]!, this.numHeads, this.keyDim]).transpose([0, 2, 1, 3]); // → [B, H, L, d_k]
