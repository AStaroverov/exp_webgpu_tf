  ...
  call(inputs: tf.Tensor | tf.Tensor[]) {
    return tf.tidy(() => {
      ...
      if (inputArray.length !== 4) {
        throw new Error(
          `SwinAttentionLayer expects exactly 4 inputs: [qTok, qMask, kvTok, kvMask], got ${inputArray.length}`,
        );
      }
      ...
      for (let i = 0; i < numWindows; i++) {
        ...
        if (actualWindowSize < this.window) {
          ...
          qMaskWin = tf.pad(qMaskWin, [
            [0, 0],
            [0, padSize],
          ]);
          kvMaskWin = tf.pad(kvMaskWin, [
            [0, 0],
            [0, padSize],
          ]);
        }
      ...
      const split = (t: tf.Tensor) =>
        t.reshape([BW, this.window, this.numHeads, this.keyDim]).transpose([0, 2, 1, 3]); // → [BW, H, windowSize, d_k]
      ...
      let scores = tf.matMul(qh, kh, false, true).div(Math.sqrt(this.keyDim)); // [BW, H, windowSize, windowSize]
      ...
      const merged = context.transpose([0, 2, 1, 3]).reshape([BW, this.window, dModel]);
      ...
      for (let i = 0; i < numWindows; i++) {
        ...
        const winOut = windowOutputs
          .slice([0, i, 0, 0], [B, 1, actualWindowSize, dModel])
...
function write(input: tf.Tensor, weight: tf.LayerVariable): tf.Tensor {
