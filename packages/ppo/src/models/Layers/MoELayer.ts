...
export class MoELayer extends tf.layers.Layer {
    ...
    call(inputs: tf.Tensor | tf.Tensor[], kwargs?: { training?: boolean }): tf.Tensor {
        ...
        return tf.tidy(() => {
            ...
            const upKernelReshaped = this.expertUpKernel.read()
                .transpose([1, 0, 2])
            ...
            const upProj = flatInput.matMul(upKernelReshaped)
            ...
            if (training && this.expertDropout > 0) {
                ...
                const expertMask = tf.randomUniform([1, this.numExperts, 1])
            ...
            const gatherIndices = tf.stack([batchIndices.flatten(), topKIndices.flatten().cast('int32')], 1);
            const gatheredOutputs = gatherND(expertOutputs, gatherIndices)
                .reshape([batchSeq, this.topK, this.inputDim]);
