...
export function applyPerceiverLayer({
  ...
  for (let i = 0; i < depth; i++) {
    ...
    const qMaskI =
      typeof qMask === "function"
        ? qMask(name, i)
        : (qMask ??
          (new MaskLikeLayer({ name: name + "_qMaskLike" + i }).apply(x) as tf.SymbolicTensor));
    const kvMaskI =
      typeof kvMask === "function"
        ? kvMask(name, i)
        : (kvMask ??
          (new MaskLikeLayer({ name: name + "_kvMaskLike" + i }).apply(
            kvTokI,
    ...
    x = tf.layers
      .add({ name: `${name}/residual/depth${i}` })
