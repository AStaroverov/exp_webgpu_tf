...
export class VariableLayer extends tf.layers.Layer {
  ...
  constructor(
    ...
    this.initializer =
      config.initializer instanceof Initializer
        ? config.initializer
        : getInitializer(config.initializer ?? "glorotNormal");
  }

  build(inputShape: tf.Shape | tf.Shape[]): void {
    this.variable = this.addWeight("variable", this.shape, "float32", this.initializer);
