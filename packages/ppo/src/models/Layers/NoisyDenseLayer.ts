...
export class NoisyDenseLayer extends tf.layers.Layer {
  ...
  constructor(config: NoisyDenseConfig) {
    ...
    this.biasInitializer =
      config.biasInitializer instanceof Initializer
  ...
  call(inputs: tf.Tensor | tf.Tensor[], kwargs?: { noise?: boolean | NoisePredicate }): tf.Tensor {
    ...
    return tf.tidy(() => {
      ...
      if (enable) {
        ...
        kernel = kernelMu.add(
          this.applySigmaToNoise(kernelNoise, this.kernelSigma.read(), "kernel"),
        );
        ...
        if (this.useBias && this.biasMu && this.biasSigma && biasNoise) {
          ...
          bias = this.biasMu
            .read()
            .add(this.applySigmaToNoise(biasNoise, this.biasSigma.read(), "bias"));
        }
      } else {
