...
export async function getNetwork(modelName: Model, savePath: string, getInitial?: () => tf.LayersModel) {
    let network: undefined | tf.LayersModel;

    try {
        network = await patientAction(() => loadLastNetworkFromDB(modelName, savePath), isFunction(getInitial) ? 1 : 10);
    } catch (error) {
...
export async function getPenultimateNetworkVersion(name: Model, savePath: string): Promise<number | undefined> {
    const defaultSubName = getStorePath(name, LAST_NETWORK_VERSION, savePath);
    const penultimate = (await getNetworkInfoList(name, savePath)).reduce((acc, info) => {
