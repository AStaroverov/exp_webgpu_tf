...
import { getIndexedDBModelPath, removeOutLimitNetworks, shouldSaveHistoricalVersion } from './storage.ts';
...
export async function saveNetworkToDB(network: tf.LayersModel, name: Model, savePath: string) {
    ...
    if (isBrowser) {
        if (shouldSaveHistorical) {
            ...
            void network.save(getIndexedDBModelPath(name, networkVersion, savePath), { includeOptimizer: true });
            void removeOutLimitNetworks(name, savePath);
        }

        return network.save(getIndexedDBModelPath(name, LAST_NETWORK_VERSION, savePath), { includeOptimizer: true });
    }
