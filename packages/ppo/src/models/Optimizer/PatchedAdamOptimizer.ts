...
export class PatchedAdamOptimizer extends AdamOptimizer {
  ...
  applyGradients(variableGradients: NamedTensor[] | NamedTensorMap) {
    ...
    // @ts-expect-error
    const accumulatedFirstMoment = this.accumulatedFirstMoment as {
      originalName: string;
      variable: Variable;
    }[];
    // @ts-expect-error
    const accumulatedSecondMoment = this.accumulatedSecondMoment as {
      originalName: string;
      variable: Variable;
    }[];
    ...
    tidy(() => {
      ...
      varNames.forEach((name, i) => {
        ...
        let firstMomentIndex = accumulatedFirstMoment.findIndex(
          ({ originalName }) => originalName === `${name}${FM_POSTFIX}`,
        );
        let secondMomentIndex = accumulatedSecondMoment.findIndex(
          ({ originalName }) => originalName === `${name}${SM_POSTFIX}`,
        );
