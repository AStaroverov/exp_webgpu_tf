...
export function createDenseLayer(
  options: DenseLayerArgs &
    Required<Pick<DenseLayerArgs, "useBias" | "activation">> & { noisy?: boolean; sigma?: number },
) {
  return options.noisy ? new NoisyDenseLayer(options) : tf.layers.dense(options);
}

// https://arxiv.org/html/2406.09079v1
export function applyLaNLayer(
  {
    name,
    units,
    preNorm = false,
...
export function applySelfTransformerLayer({
  ...
  const attnResidual = tf.layers
    .add({ name: `${name}_residual` })
  ...
  const ffnInner = tf.layers
    .multiply({ name: `${name}_ffn_silu` })
 ...
 * leaves unnormalized).
 */
export function applySelfTransformLayers(
  name: string,
  {
    depth,
    heads,
...
export function applyGlobalAverage1d({ name }: { name: string }, token: tf.SymbolicTensor) {
  return tf.layers
    .globalAveragePooling1d({ name: name + "_GlobalAvgPool1D" })
