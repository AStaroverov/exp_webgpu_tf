...
export function createPolicyLearnerAgent<S>({
  config,
  createInputTensors,
  ...
  const trainPolicy = (network: tf.LayersModel, batch: LearnData<S>) => {
    ...
    for (let i = 0; i < config.policyEpochs(expIteration); i++) {
      for (let j = 0; j < mbc; j++) {
        ...
        const tActions = tf.tensor2d(flatTypedArray(mBatch.actions), [
          mBatch.actions.length,
          mBatch.actions[0].length,
        ]);
    ...
    return onReadyRead()
      ...
      .then(([klList, policyLossList]) => {
        if (policyLossList.some((v) => isLossDangerous(v, 1000))) {
          throw new Error(
            `Policy loss too dangerous: ${min(...policyLossList)}, ${max(...policyLossList)}`,
          );
        }
        ...
        console.info(
          `[Train Policy]: Finish iteration=${expIteration}, entropy=${avgEntropy.toFixed(4)}`,
        );
      });
  };
  ...
  const computeKLForBatch = (
    ...
    if (batch.states.length > 0) {
      ...
      const tActions = tf.tensor2d(flatTypedArray(batch.actions), [
        batch.actions.length,
        batch.actions[0].length,
      ]);
      ...
      result = computeKullbackLeiblerAprox(network, tStates, tActions, tLogProb, mbs, tMasks);
