...
export function createValueLearnerAgent<S>({
  config,
  createInputTensors,
  ...
  const trainValue = (network: tf.LayersModel, batch: LearnData<S>) => {
    ...
    return onReadyRead()
      .then(() => Promise.all(valueLossList.map((t) => asyncUnwrapTensor(t).then((v) => v[0]))))
      .then((valueLossList) => {
        console.info(`[Train Value]: Finish`);

        if (valueLossList.some((v) => isLossDangerous(v, 1000))) {
          throw new Error(
            `Value loss too dangerous: ${min(...valueLossList)} ${max(...valueLossList)}`,
          );
        }
