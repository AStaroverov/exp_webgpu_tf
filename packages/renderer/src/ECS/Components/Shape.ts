...
type ShapeComponent = {
    kind: Uint8Array;
    values: ReturnType<typeof NestedArray.f64>;
    addComponent: (world: World, id: number, k?: ShapeKind, a?: number, b?: number, c?: number, d?: number, e?: number, f?: number) => void;
    setCircle$: (id: number, radius: number) => void;
    setRectangle$: (id: number, width: number, height: number) => void;
    setParallelogram$: (id: number, width: number, height: number, skew: number) => void;
    setTrapezoid$: (id: number, topWidth: number, bottomWidth: number, height: number) => void;
    setTriangle$: (id: number, a: number, b: number, c: number, d: number, e: number, f: number) => void;
};

export const createShapeComponent = defineComponent<ShapeComponent>(({ ref: Shape, obs }) => ({
