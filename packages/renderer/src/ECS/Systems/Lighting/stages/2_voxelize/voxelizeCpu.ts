...
// Build per-instance voxel AABBs + the prefix-sum work list for the scatter pass, for ALL clipmap
// levels in ONE flat list: level L's boxes occupy entries [L*n, (L+1)*n) ("virtual instances" —
// the scatter shader decodes level = vins / n, real instance = vins % n) and the prefix runs
// continuously across levels, so one dispatch scatters every level. For each (level, instance):
// compute a CONSERVATIVE rotated-box world AABB, convert it to a voxel box clamped to that level's
// grid, and accumulate the prefix sum of voxel counts. The scatter shader walks the flat list via
// a binary search on `start`. aabbMin.w = prefix start; aabbDim.w = voxel count. Returns
// scatterTotal (the grand total of scattered voxels = the last prefix). A coarse level's per-
// instance voxel count is ~1/8 of the fine level's (cell 2× per axis), so extra levels add ~12%
// work, not 2× — plus the instances only the coarse box covers.
export function buildVoxelAABBs(
  ...
  let prefix = 0;
  for (let lvl = 0; lvl < boxes.length; lvl++) {
    const { originX, originY, originZ, cellSize, dimX, dimY, dimZ } = boxes[lvl];
    const base = lvl * n;
    for (let k = 0; k < n; k++) {
      ...
      const i = base + k;
      aabbMinArr[i * 4 + 0] = vx0;
      aabbMinArr[i * 4 + 1] = vy0;
      aabbMinArr[i * 4 + 2] = vz0;
      aabbMinArr[i * 4 + 3] = prefix;
      aabbDimArr[i * 4 + 0] = nx;
      aabbDimArr[i * 4 + 1] = ny;
      aabbDimArr[i * 4 + 2] = nz;
      aabbDimArr[i * 4 + 3] = count;
      prefix += count;
    }
  }
  return prefix;
