...
export const createImpulseComponent = defineComponent((Impulse) => {
    ...
    return {
        ...
        add(eid: EntityId, ax: number, ay: number) { x[eid] += ax; y[eid] += ay; },
        set(eid: EntityId, sx: number, sy: number) { x[eid] = sx; y[eid] = sy; },
        reset(eid: EntityId) { x[eid] = 0; y[eid] = 0; },
        hasImpulse(eid: EntityId): boolean { return x[eid] !== 0 || y[eid] !== 0; },
    };
});
...
export const createTorqueImpulseComponent = defineComponent((TorqueImpulse) => {
    ...
    return {
        ...
        add(eid: EntityId, torque: number) { value[eid] += torque; },
        set(eid: EntityId, torque: number) { value[eid] = torque; },
        reset(eid: EntityId) { value[eid] = 0; },
        hasImpulse(eid: EntityId): boolean { return value[eid] !== 0; },
    };
});
...
export const createImpulseAtPointComponent = defineComponent((ImpulseAtPoint) => {
    ...
    return {
        ...
        reset(eid: EntityId) { count[eid] = 0; },
        hasImpulse(eid: EntityId): boolean { return count[eid] > 0; },
    };
});
