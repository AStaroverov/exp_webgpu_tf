...
export function createApplySensorHitsSystem({ world } = GameDI) {
    const {
        SensorHits, Damagable, Dotable, Dot, VehiclePart, Obstacle, DestroyByTimeout, PlayerRef, Hitable,
        ...
        for (let i = 0; i < projectileEids.length; i++) {
            ...
            for (let j = 0; j < hitCount; j++) {
                ...
                if (hasComponent(world, otherEid, VehiclePart)) {
                    ...
                    if (hasComponent(world, otherEid, PlayerRef) && PlayerRef.id[otherEid] === PlayerRef.id[projectileEid]) continue;
                    ...
                    if (hasComponent(world, otherEid, Hitable)) {
                        Hitable.hit$(otherEid, projectileEid, Damagable.damage[projectileEid], Damagable.kind[projectileEid]);
                    }
                    ...
                    if (hasComponent(world, projectileEid, Dotable)) {
                        ...
                        Dot.addComponent(world, otherEid, dps, Dotable.durationMs[projectileEid], Dotable.kind[projectileEid]);
                    }

                    // Pass-through decay: each enemy-part overlap eats the projectile's lifetime.
                    DestroyByTimeout.resetTimeout(projectileEid, max(0, DestroyByTimeout.timeout[projectileEid] - SensorHits.hitLifeCostMs[projectileEid]));
