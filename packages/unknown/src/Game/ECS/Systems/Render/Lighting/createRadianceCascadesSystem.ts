...
export function createRadianceCascadesSystem({ device, params, frameTextures, sceneTexture, drawEmitters }: {
    ...
    const seedPipeline = seedShader.getRenderPipeline(device, 'vs_main', 'fs_main', { targetFormat: 'rg16float', withBlending: false });
    ...
    const jfaStages: { shader: GPUShader<typeof jfaMeta>, pipeline: GPURenderPipeline, bindGroup: GPUBindGroup, target: GPUTexture }[] = [];
    for (let i = 0; i < jfaPasses; i++) {
        ...
        const pipeline = shader.getRenderPipeline(device, 'vs_main', 'fs_main', { targetFormat: 'rg16float', withBlending: false });
        ...
        device.queue.writeBuffer(shader.uniforms.oneOverSize.getGPUBuffer(device), 0, oneOverSizeBuffer);
    ...
    const dfPipeline = dfShader.getRenderPipeline(device, 'vs_main', 'fs_main', { targetFormat: 'r16float', withBlending: false });
    ...
    const cascadeStages: { shader: GPUShader<typeof rcMeta>, pipeline: GPURenderPipeline, bindGroup: GPUBindGroup, target: GPUTexture }[] = [];
    let writeSlot = 0;
    for (let c = cascadeCount - 1; c >= 0; c--) {
        ...
        device.queue.writeBuffer(shader.uniforms.resolution.getGPUBuffer(device), 0, resolutionBuffer);
        device.queue.writeBuffer(shader.uniforms.cascadeIndex.getGPUBuffer(device), 0, cascadeIndexBuffer);
    ...
    function pass(encoder: GPUCommandEncoder, target: GPUTexture, pipeline: GPURenderPipeline, bindGroup: GPUBindGroup, draw: (renderPass: GPURenderPassEncoder) => void) {
...
function writeVec4(device: GPUDevice, shader: GPUShader<any>, key: string, rgb: [number, number, number], mult: number, w: number) {
    const buffer = getTypeTypedArray(shader.uniforms[key].variable.type);
    buffer[0] = rgb[0] * mult;
    buffer[1] = rgb[1] * mult;
    buffer[2] = rgb[2] * mult;
