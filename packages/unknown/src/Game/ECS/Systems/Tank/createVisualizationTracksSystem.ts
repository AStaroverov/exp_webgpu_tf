...
import { getNodeByPhysics, getNodeSlots, getOccupantOf } from '../../refs.ts';
import { Worlds } from '../../../DI/Worlds.ts';

const TURN_FACTOR = 0.7;
const ANGULAR_SCALE = 50;

export function createVisualizationTracksSystem({ physicsWorld, slotWorld, physicalWorld } = Worlds) {
    ...
    return (_delta: number) => {
        ...
        for (const trackEid of trackEids) {
            ...
            for (const slotEid of getNodeSlots(getNodeByPhysics(trackEid))) {
                ...
                const fillerEid = getOccupantOf(slotEid);
