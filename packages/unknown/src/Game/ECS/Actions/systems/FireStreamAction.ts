 ...
 *   2. HOLDING — raise the shoot flag every tick and accumulate `elapsed`; at
...
export function createFireStreamActionSystem({ world } = GameDI) {
    const { ActionsQueue, Vehicle, Tank, TurretController, VehicleController, HullAimed, StreamFirearms, RigidBodyState } = getGameComponents(world);
        ...
        for (const ownerEid of eids) {
            ...
            if (phase === FIRE_STREAM_PHASE_AIMING) {
                ...
                if (aim === AIM_ON_TARGET) {
                    ...
                    ActionsQueue.setParam(ownerEid, 0, FireStreamParamOffset.phase, FIRE_STREAM_PHASE_HOLDING);
                }
                continue;
            }
            ...
            if (elapsed >= cfg.holdMs) {
                ...
                const nextIsStream = ActionsQueue.count[ownerEid] > 1
