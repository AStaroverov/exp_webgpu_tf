...
export function topK(x: tf.Tensor, k: number): { values: tf.Tensor; indices: tf.Tensor } {
    ...
    const customTopKValues = tf.customGrad((
        input: tf.Tensor | GradSaveFunc,
        save: tf.Tensor | GradSaveFunc,
    ) => {
        ...
        const gatherIndices = tf.stack([
            batchIndices.flatten(),
            indices.flatten().cast('int32')
        ], 1);
        ...
        const gradFunc = (dy: tf.Tensor, saved: tf.Tensor[]): tf.Tensor => {
            ...
            const batchIdx = tf.range(0, batchSize, 1, 'int32')
                .expandDims(1)
                .tile([1, k]);
            const scatterIndices = tf.stack([
                batchIdx.flatten(),
                savedIndices.flatten().cast('int32')
            ], 1);
