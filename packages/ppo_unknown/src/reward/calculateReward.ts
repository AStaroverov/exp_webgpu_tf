 ...
 *
 *   calculateActionReward(eid)     — cumulative COMBAT score for the tank's player; the
 *                                    agent deltas it into a REAL per-macro-action reward
 *                                    (never annealed) — this is the dense objective.
 *   calculateShapingPotential(eid) — cumulative APPROACH-shaping score; the agent uses it
 *                                    as a potential γ·Φ(s') − Φ(s) (policy-invariant) and
 *                                    fades it with `shapingWeight`. Safe to anneal to zero.
...
/**
 * Cumulative APPROACH-shaping score for the tank's player (the potential Φ). The agent
 * forms the shaping reward as γ·Φ(s') − Φ(s) and scales it by `shapingWeight`, so it is
 * policy-invariant w.r.t. the combat + terminal objective and can be annealed to zero.
 */
export function calculateShapingPotential(eid: number, { world } = GameDI): number {
  const { PlayerRef } = getGameComponents(world);
  return clamp(scoreTracker.getShapingScore(PlayerRef.id.get(eid)), -REWARD_LIMIT, REWARD_LIMIT);
}
