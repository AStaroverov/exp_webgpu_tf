...
(`@dimforge/rapier3d-simd`) with the **renderer** (2.5D SDF) renderer over a
...
renderer is consumed by **relative path** (`../../renderer/src/...`), exactly
...
- **Render** — renderer, a **2.5D SDF impostor** renderer with Voxel Cone Tracing
...
(`packages/renderer/src/ECS/Systems/SDFSystem/sdf.shader.ts` +
...
We cannot call renderer's `createWorld()` directly: it builds a world whose
...
export function getEngineComponents(world): EngineComponents {
  /* throws if absent */
}
```

**Important nuance discovered in source:** in renderer `GlobalTransform` /
...
reference. renderer's own systems (`createDrawShapeSystem`, `createTransformSystem`,
...
why our world is drop-in compatible with every renderer system** — they only ever
...
| ----------------------------------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `GlobalTransform`, `LocalTransform`                         | **Reuse** (renderer `Transform.ts`) | The render pass reads `GlobalTransform.matrix.getBatch(eid)`. The sync system must write into the exact same column the renderer reads. `matrix` is `NestedArray.f32(16)` → a `mat4` view. Reusing is mandatory, not optional. |
...
for (let i = 0; i < entities.length; i++) {
  ...
  quat.set(
    _q,
    RBS.rotation.get(eid, 0),
    RBS.rotation.get(eid, 1),
    RBS.rotation.get(eid, 2),
    RBS.rotation.get(eid, 3),
  );
  vec3.set(_t, RBS.position.get(eid, 0), RBS.position.get(eid, 1), RBS.position.get(eid, 2) - hz); // baseZ = centerZ − hz
...
1. physicalFrame(delta):
   a. execTransformSystem()              // (renderer TransformSystem) local→global compose
...
The **render block** (`renderFrame`, built in `createRenderTarget`) is the renderer
...
sync systems, then the renderer pass chain ending in `present(...)`.
...
1. **`Physics/initPhysicalWorld.ts`** — `initPhysicalWorld(): PhysicalWorld`,
   ...
   lengthUnit/solver tuning. _(deps: rapier3d-simd)_
2. **`Physics/createBody.ts`** — `createBody(world, {type,x,y,z,rot}): RigidBody`,
   ...
   `setTranslation(x,y,z)`, `setRotation(quat)`. _(deps: rapier3d-simd)_
3. **`Physics/createRigid.ts`** — `createRigidBox(world, body, hx,hy,hz): number`,
   ...
   `ColliderDesc.cuboid(hx,hy,hz)` / `.ball(r)`. _(deps: rapier3d-simd)_
...
eid, pid)`, `clear(eid)`, `dispose()`; module `Map<pid,eid>` +
   `getEntityIdByPhysicalId(pid): number`. _(deps: renderer ECS/utils
   `defineComponent`, utils `TypedArray`)_
...
NestedArray.f64(3,size)`; `addComponent(world,eid)` (zero-fills, identity quat at
   rotation[3]=1), `update(eid, px,py,pz, qx,qy,qz,qw, lx,ly,lz, ax,ay,az)`. _(deps:
   renderer ECS/utils, utils `NestedArray`)_
6. **`ECS/components.ts`** — `createEngineComponents(world)` returns
   `{ RigidBodyRef: createRigidBodyRefComponent(world), RigidBodyState3D:
createRigidBodyState3DComponent(world) }`. _(deps: 4, 5)_
7. **`ECS/createEngineWorld.ts`** — `createEngineWorld(): EngineWorld`,
   ...
   `{components, time}` context. _(deps: 6, renderer `ECS/world.ts`)_
8. **`ECS/Systems/createRigidBodyStateSystem.ts`** —
   ...
   `translation/rotation/linvel/angvel`, `RigidBodyState3D.update(...)`. _(deps: 5, 7,
   rapier3d-simd)_
...
RigidBodyRef, RigidBodyState3D, Height]`, `mat4.fromRotationTranslation` with
   bottom-Z offset → `LocalTransform.matrix.getBatch(eid)`. _(deps: 5, 7, renderer
   `Transform.ts`, gl-matrix)_
10. **`ECS/Entities/RigidShapes.ts`** — `createRigidBox`, `createRigidSphere`,
    ...
    _(deps: 2, 3, 7, renderer `Entities/Shapes.ts`)_
11. **`DI/RenderDI.ts`** — the `RenderDI` singleton (§2). _(no deps)_
12. **`DI/EngineDI.ts`** — the `EngineDI` singleton (§2). _(deps: 7 for types)_
...
depthTexture,normalTexture,albedoTexture:renderTexture,emissionTexture})`,
    ...
    block (incl. resize rebuild + `voxel.recreate`). Fills `RenderDI`. _(deps: 11,
    many renderer modules)_
14. **`createEngine.ts`** — `createEngine({canvas, width?, height?}): Promise<EngineDI>`.
    ...
    and `createTransformSystem` (renderer), assembles `tick(delta)` =
    `physicalFrame` + `RenderDI.renderFrame?.(delta)`, wires `setRenderTarget` (calls
    13), fills `EngineDI`. _(deps: 1, 7, 8, 9, 12, 13, renderer `TransformSystem`)_
15. **`demo.ts`** — entry (referenced by `index.html`). `createEngine`, build scene
    (10) + sun + camera, run the rAF loop. _(deps: 14, 10, renderer `SunLight`,
    `ResizeSystem` camera setters)_
...
4. **Two systems named "renderer" vs "renderer".** The engine imports **only**
   `renderer`. Ensure no accidental `../../renderer/...` imports leak in (the old 2D
...
6. **`createRenderComponents` returns module-singleton transforms.** `GlobalTransform`/
   ...
   in one tab is a known renderer limitation, not introduced by the engine.
