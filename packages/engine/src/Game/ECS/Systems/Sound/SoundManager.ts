...
class SoundManager {
  ...
  play(id: string, options: PlayOptions = {}): WebAudioTrack | null {
    ...
    const randomSrcIndex = buffers.length > 1 ? Math.floor(Math.random() * buffers.length) : 0;
    ...
    instance.track.setBuffer(buffers[randomSrcIndex]).setLoop(options.loop ?? config.loop ?? false);
