...
export function createDrawGrassSystem({ canvas, device } = RenderDI) {
  ...
  device.queue.writeBuffer(
    gpuShader.uniforms.grassDensity.getGPUBuffer(device),
    0,
    grassDensityBuffer,
  );
  ...
  return (renderPass: GPURenderPassEncoder, delta: number) => {
    ...
    windStrengthBuffer[0] =
      (1 + sin(windStrengthBuffer[0] + (randomSign() * random() * delta) / 100)) / 2;
    ...
    device.queue.writeBuffer(
      gpuShader.uniforms.screenSize.getGPUBuffer(device),
      0,
      screenSizeBuffer,
    );
    device.queue.writeBuffer(gpuShader.uniforms.mapOffset.getGPUBuffer(device), 0, mapOffsetBuffer);
    device.queue.writeBuffer(
      gpuShader.uniforms.windStrength.getGPUBuffer(device),
      0,
      windStrengthBuffer,
    );
    device.queue.writeBuffer(
      gpuShader.uniforms.windDirection.getGPUBuffer(device),
      0,
      windDirectionBuffer,
    );
